...
parser = argparse.ArgumentParser(description='HistologyNucleiPixels Training')
...
def main():
    ...
    wandb.init(project="HistologyNucleiPixels", name=save_dir)
    ...
    train_dataset = dataset.DatasetTrain(DATA_PATHS[args.data_loc]["HistologyNucleiPixels"])
    train_loader = torch.utils.data.DataLoader(dataset=train_dataset, batch_size=32, shuffle=True)

    val_dataset = dataset.DatasetVal(DATA_PATHS[args.data_loc]["HistologyNucleiPixels"])
    val_loader = torch.utils.data.DataLoader(dataset=val_dataset, batch_size=32, shuffle=True)

    ood_dataset = dataset.DatasetTest(DATA_PATHS[args.data_loc]["HistologyNucleiPixels"])
    ...
    for i, (xs, ys) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            print('Epoch: [{0}][{1}/{2}]\t'
                  'Loss: {losses.avg:.4f}\t'
                  'Var: {var:.4f}\t'.format(
                      epoch, i, len(train_loader), losses=losses, var=np.e**(log_vars.avg)))

            wandb.log({f'train/loss': float(losses.avg)}, step=epoch*len(train_loader)+i)
            wandb.log({f'train/var': float(np.e**(log_vars.avg))}, step=epoch*len(train_loader)+i)
            losses = AverageMeter()
            log_vars = AverageMeter()
...
def validate(val_loader, model, epoch, name):
    ...
    log_vars = AverageMeter()
    mses = AverageMeter()
    dist_from_data_mean = AverageMeter()
    ...
    with torch.no_grad():
        for i, (xs, ys) in enumerate(val_loader):
            ...
            dist_from_data_mean.update(torch.mean(torch.pow(means, 2)).item(), xs.size(0))

    print('{name} Epoch: [{epoch}]\t'
        'Loss: {losses.avg:.4f}\t'
        'MSE: {mses.avg:.3f}\t'
        'Var: {var:.4f}\t'
        'Dist from data mean: {dist_from_data_mean.avg:.4f}'.format(epoch=epoch,
            name=name, losses=losses, mses=mses, var=np.e**(log_vars.avg), dist_from_data_mean=dist_from_data_mean))

    wandb.log({f'{name}/loss': float(losses.avg)})
    wandb.log({f'{name}/var': float(np.e**(log_vars.avg))})
    wandb.log({f'{name}/mse': float(mses.avg)})
    wandb.log({f'{name}/dist_from_data_mean': float(dist_from_data_mean.avg)})
